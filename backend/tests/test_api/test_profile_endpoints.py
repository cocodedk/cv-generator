...
class TestDeleteProfile:
    ...
    async def test_delete_profile_requires_confirmation_header(
        ...
        with patch("backend.database.queries.delete_profile", return_value=True) as mock_delete:
...
class TestListProfiles:
    ...
    async def test_list_profiles_success(self, client, mock_neo4j_connection):
        ...
        with patch("backend.database.queries.list_profiles", return_value=profiles_data):
...
class TestGetProfileByUpdatedAt:
    ...
    async def test_get_profile_by_updated_at_success(self, client, mock_neo4j_connection):
        ...
        with patch(
            "backend.database.queries.get_profile_by_updated_at", return_value=profile_data
        ):
    ...
    async def test_get_profile_by_updated_at_not_found(self, client, mock_neo4j_connection):
        """Test profile retrieval by updated_at when not found."""
        with patch("backend.database.queries.get_profile_by_updated_at", return_value=None):
            response = await client.get("/api/profile/2024-01-01T00:00:00")
            assert response.status_code == 404

    async def test_get_profile_by_updated_at_server_error(self, client, mock_neo4j_connection):
...
class TestDeleteProfileByUpdatedAt:
    ...
    async def test_delete_profile_by_updated_at_success(self, client, mock_neo4j_connection):
        """Test successful profile deletion by updated_at."""
        with patch("backend.database.queries.delete_profile_by_updated_at", return_value=True):
            response = await client.delete(
    ...
    async def test_delete_profile_by_updated_at_not_found(self, client, mock_neo4j_connection):
        """Test delete non-existent profile by updated_at."""
        with patch("backend.database.queries.delete_profile_by_updated_at", return_value=False):
            response = await client.delete(
                "/api/profile/2024-01-01T00:00:00",
                headers={"X-Confirm-Delete-Profile": "true"},
            )
    ...
    async def test_delete_profile_by_updated_at_server_error(self, client, mock_neo4j_connection):
