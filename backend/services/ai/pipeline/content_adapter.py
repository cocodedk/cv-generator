...
"""
    ...
    async def adapt_with_fallback(task_info: Tuple[str, str, str, str, str]) -> Tuple[str, str, str, str, str, str, Optional[str]]:
    ...
    adaptation_results = await asyncio.gather(
        *[adapt_with_fallback(task) for task in adaptation_tasks],
    ...
    for exp_idx, exp in enumerate(selection_result.experiences):
        logger.debug(f"Reconstructing experience {exp_idx+1}/{len(selection_result.experiences)}: {exp.title} at {exp.company}")
