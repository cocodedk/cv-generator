    ...
    for experience in cv.experience:
        ...
        for project in experience.projects or []:
            rewritten_highlights = [
                _rewrite_highlight(highlight)
                for highlight in (project.highlights or [])
            ]
            rewritten_projects.append(
                Project(
                    **project.model_dump(exclude={"highlights"}),
                    highlights=rewritten_highlights,
                )
            )
        rewritten_experiences.append(
            Experience(
                **experience.model_dump(exclude={"projects"}),
